...
**使用的 Skills**:
- **skill-logic-auditor**: 审计 Skill 逻辑缺陷（防幻觉、知识库利用）
- **plugin-auditor**: 审查 Plugin 配置文件和架构合理性
...
### 步骤 8: 调用 skill-logic-auditor 审计逻辑缺陷

**对于 SKILL.md 文件，必须调用 skill-logic-auditor 进行深度逻辑审计**：

```
Skill(
  skill: "skill-logic-auditor",
  args: {
    "target_path": "{target_path}",
    "plugin_path": "{plugin_path}",
    "focus_areas": ["anti_hallucination", "kb_utilization", "input_validation"]
  }
)
```

skill-logic-auditor 会检查：
- 信息缺失时是否强行输出结论（防幻觉）
- 是否充分利用知识库（RAG 集成）
- 输入校验是否完整

### 步骤 9: 如果需要 Plugin 架构审查，调用 plugin-auditor

**当测试范围涉及整个 Plugin 或需要检查配置文件时**：
...
**依赖Skill**: skill-logic-auditor（逻辑审计）、plugin-auditor（架构审计）
**v1.1 更新**: 集成 skill-logic-auditor 和 plugin-auditor 进行深度审计
