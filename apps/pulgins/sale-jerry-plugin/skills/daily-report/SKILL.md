...
| 上线运维类 | `上线`、`部署`、`运维`、`巡检`、`故障处理`、`故障排查` | 售后上线运维阶段工作 |
| 项目管理类 | `甘特图`、`项目计划`、`里程碑`、`排期` | 项目经理排期计划工作 |
| 定制开发类 | `定制化开发`、`定制化需求`、`调研清单`、`开发任务` | 售后定制化开发工作 |
| 测试验证类 | `性能测试`、`专项测试`、`UAT`、`测试计划`、`测试用例`、`集成测试`、`联调测试` | 售后测试验证阶段工作 |
| 文档迭代类 | `实施方案.*迭代`、`V\d+\.\d+.*升级`、`文档格式统一` | 交付文档多版本迭代工作 |
| 研发工程类 | `CI/CD`、`流水线`、`制品`、`自动化脚本`、`研发`、`开发调试`、`脚本开发` | 研发工程与内部工具开发 |
| 系统集成类 | `系统对接`、`接口对接`、`环境部署`、`动环`、`联调` | 系统集成与接口联调工作 |
| 技术保障类 | `技术支持`、`技术保障`、`风险管理`、`进度管理`、`自动化` | 技术保障与项目管控工作 |
...
function filter_daily_report_sessions(sessions):
    ...
    filter_keywords = [
        ...
        "文档格式统一", "发布管理",
        # --- 研发工程类 ---
        "CI/CD", "流水线", "制品", "自动化脚本", "研发",
        "开发调试", "脚本开发",
        # --- 系统集成类 ---
        "系统对接", "接口对接", "环境部署", "动环", "联调",
        # --- 技术保障类 ---
        "技术支持", "技术保障", "风险管理", "进度管理", "自动化",
    ]
    ...
    filter_patterns = [
        ...
        # --- 研发工程模式 ---
        r"CI.?CD", "流水线", r"制品.*上传", r"脚本.*开发", r"研发.*管理",
        # --- 系统集成模式 ---
        r"系统.*对接", r"接口.*对接", r"环境.*部署",
        # --- 测试联调模式 ---
        r"集成.*测试", r"联调.*测试",
        # --- 技术保障模式 ---
        r"风险.*管理", r"进度.*管理", r"技术.*支持", r"技术.*保障",
    ]
...
* 日报系统验证、测试相关的会话
* 包含"日报生成"、"生成日报"关键词的会话
* 包含"交付"、"实施"、"上线"、"运维"、"甘特图"、"定制化开发"、"测试计划"、"CI/CD"、"流水线"、"系统对接"、"技术支持"等关键词的售后及研发阶段会话
...
| `上线/运维/巡检/故障处理/故障排查` | 售后运维阶段 |
| `甘特图/项目计划/里程碑/排期` | 项目管理阶段 |
| `定制化开发/定制化需求/调研清单` | 售后定制开发 |
| `性能测试/专项测试/测试计划/UAT/集成测试/联调测试` | 售后测试验证 |
| `CI/CD/流水线/制品/自动化脚本/研发/脚本开发` | 研发工程 |
| `系统对接/接口对接/环境部署/动环/联调` | 系统集成 |
| `技术支持/技术保障/风险管理/进度管理/自动化` | 技术保障 |
...
| 版本 | 日期 | 说明 |
|------|------|------|
| v4.15 | 2026-06-03 | 步骤 3.5 新增研发工程类（CI/CD、流水线、制品、自动化脚本、研发）、系统集成类（系统对接、接口对接、环境部署、动环、联调）、技术保障类（技术支持、技术保障、风险管理、进度管理）过滤规则，补充故障排查、集成测试、联调测试等关键词 |
