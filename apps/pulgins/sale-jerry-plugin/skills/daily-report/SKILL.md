...
**日期规则**：
- 日报日期始终为**前一个日历日**（昨天）
- 周六执行 → 生成周五的日报（昨天=周五，有会话记录）
- 周日执行 → 不生成（昨天=周六，无工作会话，自然终止）
- 周一执行 → 不生成（昨天=周日，无工作会话，自然终止）
- 以实际会话内容日期为准，无会话则不生成
...
5. **以实际日期为准**：日报日期=前一个日历日，无会话则自然终止（不额外回溯工作日）
...
**日期规则说明**：
- 日报日期始终为前一个日历日，不判断今天是否为周末
- 周六执行 → 昨天=周五 → 查询周五会话 → 有记录则生成
- 周日执行 → 昨天=周六 → 查询周六会话 → 无记录则自然终止
- 周一执行 → 昨天=周日 → 查询周日会话 → 无记录则自然终止
...
```markdown
📅 正在生成 2026-04-06 的工作日报...
🔍 正在从 claude-mem 获取 2026-04-06 的会话记录...
ℹ️ 2026-04-06（周日）无会话记录，不生成日报。
...
**系统执行**：
1. 确定日期：2026-04-03（周五）
2. 调用 claude-mem 获取周五的会话记录
3. 分析会话，生成日报
...
**输出**：
```
📅 正在生成 2026-04-05 的工作日报...
🔍 正在从 claude-mem 获取 2026-04-05 的会话记录...
ℹ️ 2026-04-05（周日）无会话记录，不生成日报。
