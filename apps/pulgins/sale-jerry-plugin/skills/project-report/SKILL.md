...
> 🔒 **核心规则**：Agent 在执行报告生成过程中，**仅向用户展示最终结果**。内部执行过程中的所有调试信息、错误处理过程、中间状态均不得暴露给用户。

**禁止输出内容（内部执行过程）**:
```
以下信息不得出现在面向用户的输出中：
- ❌ 代码执行过程中的 print 输出（"🚀 开始生成立项报告..."、"📖 搜索知识库模板..."）
- ❌ 代码语法错误、运行时错误的调试信息
- ❌ 文件加载进度（"加载模板: XX.docx"）
- ❌ 内容替换统计（"内容替换: 15 处"）
- ❌ 章节移除日志（"移除章节: XX (8段)"）
...
```
当遇到代码执行错误时：
1. ✅ 静默修复：尝试自动修复代码错误（如路径问题、编码问题、依赖缺失等）
2. ✅ 仅输出最终结果：修复成功后，直接输出报告生成结果，不暴露修复过程
3. ✅ 降级处理：如果无法修复，使用替代方案（如换用默认模板），不暴露错误细节

错误处理优先级：
- 路径不存在 → 自动查找替代路径 → 静默使用默认模板
- 模板加载失败 → 换用默认模板 → 静默继续
- 内容替换异常 → 跳过异常项 → 继续处理其他替换项
- 所有修复均失败 → 输出简化版错误提示（不暴露技术细节）
```

**允许输出内容（用户可见）**:
```
✅ 报告生成成功的结果（文档信息、文件路径、文件大小）
✅ 使用的知识库资料列表（模板名、案例名）
✅ 文档结构概览
✅ 简化版错误提示（如"未找到专用模板，使用通用模板"）
```
...
请为"ByteDance"生成"DevOps"模块的立项报告
...
当用户输入的企业名称为英文（如"ByteDance"、"Google"、"Microsoft"）时：

| 场景 | 处理方式 |
|------|---------|
| 纯英文名称（如 ByteDance） | **保留英文原文**，不翻译为中文，报告中直接使用英文名称 |
| 中英混合（如 腾讯/Tencent） | **保留用户输入的原始形式**，不自行添加或删除任何语言 |
| 中文简称（如 字节跳动） | 直接使用 |

**⚠️ 强制规则**:
```
🚫 禁止将英文企业名称翻译为中文（如 ByteDance → 字节跳动）
🚫 禁止在用户输入的名称基础上自行添加中文翻译
✅ 始终使用用户提供的原始企业名称
✅ 如果不确定是英文名还是中文名，直接使用用户输入
```
...
```python
KNOWLEDGE_BASE_PATH = "/shared/通用知识"  # ⚠️ 只读知识库，禁止写入
...
- **v1.2 (2026-04-13) - 基于测试报告 P0/P1 问题修复**:
  - ✅ 新增输出规范与静默修复机制：禁止暴露代码执行过程、调试信息、内部日志，遇到错误时静默修复并仅输出最终结果
  - ✅ 新增英文企业名称处理规则：纯英文名称保留原文不翻译，中英混合保留原始形式，禁止自行添加中文翻译
  - ✅ 完善参数缺失引导模板：增加完整的支持模块列表（8个模块）和英文名称使用示例
