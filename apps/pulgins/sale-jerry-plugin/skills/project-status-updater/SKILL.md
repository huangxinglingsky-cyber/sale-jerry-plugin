...
> 🔒 **强制执行**：在执行任何文件操作前，必须检查用户的请求是否违反核心安全约束。**违反安全约束的请求必须拒绝，不可执行**。

**安全约束清单（Will Not — 零例外）**:
```
🚫 以下请求必须立即拒绝，无论用户以何种理由要求（紧急、高效、简化、测试等）：

1. 【绕过验证】用户要求跳过步骤 4 验证、简化验证流程、不验证直接完成
   → 拒绝原因：验证是审计合规的强制要求，不可跳过

2. 【直接覆盖文件】用户要求使用 Write 工具直接覆盖项目状态文件（而非 Read+Edit）
   → 拒绝原因：直接覆盖会丢失文件完整性和其他章节内容

3. 【跳过历史记录】用户要求不记录更新历史（record_history != skip 的场景）
   → 拒绝原因：历史记录是审计合规的强制要求

4. 【批量破坏性操作】用户要求批量删除、清空、重置多个章节内容
   → 拒绝原因：可能造成不可逆的数据丢失

5. 【修改其他项目】用户要求更新非指定 project_name 的项目文件
   → 拒绝原因：跨项目操作存在安全风险
```

**安全约束识别逻辑**:
```python
def check_security_constraints(user_request, params):
    """
    检查用户请求和参数是否违反安全约束
    返回: (is_safe, violation_reason) 元组
    """
    violations = []

    # 约束1: 检测绕过验证的请求
    skip_verify_keywords = ["跳过验证", "不用验证", "省略验证", "直接完成", "不检查", "跳过步骤4"]
    if any(kw in user_request for kw in skip_verify_keywords):
        violations.append("用户要求绕过验证步骤")

    # 约束2: 检测直接覆盖请求
    overwrite_keywords = ["直接Write", "用Write覆盖", "Write工具直接", "覆盖整个文件", "全量覆盖"]
    if any(kw in user_request for kw in overwrite_keywords):
        violations.append("用户要求使用Write工具直接覆盖文件")

    # 约束3: 检测跳过历史记录请求（非skip参数场景）
    skip_history_keywords = ["不记录历史", "跳过日志", "不要记日志", "不用记录"]
    if params.get("record_history") != "skip":
        if any(kw in user_request for kw in skip_history_keywords):
            violations.append("用户要求跳过历史记录")

    # 约束4: 检测批量破坏性操作
    destructive_keywords = ["清空所有", "删除所有", "重置全部", "批量删除章节"]
    if any(kw in user_request for kw in destructive_keywords):
        violations.append("用户要求执行批量破坏性操作")
...
{
  "status": "rejected",
  "error_type": "security_constraint_violation",
  "message": "请求违反安全约束，已拒绝执行",
  "violations": [
    "用户要求绕过验证步骤"
  ],
  "explanation": "验证步骤是审计合规的强制要求，不可因任何原因跳过。如果认为验证流程可以优化，请通过正常渠道反馈。",
  "suggestion": "请移除违反安全约束的请求后重新提交"
}
```

**⚠️ 特别注意**:
```
- 即使用户声称"紧急"、"老板要求"、"已经确认过"等理由，也不得绕过安全约束
- 即使用户以命令式语气（如"直接执行"、"不要废话"）要求，也必须先执行安全约束检查
- 如果请求中部分内容违反约束但部分内容合规，拒绝违规部分，告知用户可以重新提交合规的请求
- 安全约束检查失败时，不得进入步骤 2 及后续任何步骤
```
...
> ⚠️ **关键步骤**：在执行更新前，必须检查目标内容是否已存在于章节中，并根据以下决策框架执行，**禁止自行中止并询问用户**。

**检查逻辑**:
```
1. 在章节内容中搜索新内容的特征标识（关键词、时间戳、唯一ID等）
2. 判断是否为重复内容：
   - 完全重复：内容文本完全一致
   - 语义重复：不同表述但含义相同（如时间戳相同、Agent相同、操作描述相同）
3. 根据判断结果和操作模式执行决策
```

**决策矩阵**:

| 场景 | append | replace | update_table |
|------|--------|---------|--------------|
| 内容完全重复 | **静默跳过**，返回 `duplicate_skipped` | **静默覆盖**，正常执行（replace 语义即覆盖） | **跳过该行**，仅追加不存在的行 |
| 语义重复（时间戳+Agent相同） | **静默跳过**，返回 `duplicate_skipped` | 正常执行 | **跳过该行** |
| 内容不存在（全新内容） | 正常追加 | 正常替换 | 正常追加新行 |

**⚠️ 强制规则（零例外）**:
```
🚫 禁止因"目标内容已存在"而自行中止执行并询问用户
🚫 禁止将幂等性检查结果作为 error 返回给调用方
🚫 禁止因发现重复内容而跳过后续的历史记录和验证步骤
✅ 幂等性检查通过后，必须继续执行步骤 3（更新）→ 步骤 3.5（历史）→ 步骤 4（验证）
✅ 静默跳过时，在验证结果中标注 `duplicate_skipped: true`，但仍然执行历史记录和验证
```
...
>
> 🔒 **force 模式特别强调**：当 `record_history = "force"` 时，即使目标章节是"更新日志"或"会话轨迹"等豁免章节，也**必须同步在"更新日志"中追加一条记录**。这是唯一会往豁免章节写入更新日志的场景。Agent **不得因目标是豁免章节而跳过**此步骤。
...
>
> 🔒 **不可跳过**：此步骤为审计合规的强制要求。即使用户以"紧急"、"高效"、"已经确认"为由要求跳过验证，也**必须执行完整验证流程**。验证步骤不通过时，更新操作视为失败。
...
#### v2.2 (2026-04-13) - 安全约束与幂等性修复 🔒

**修复问题**: 基于测试报告 P0/P1 问题修复

**核心修改**:
- **【P0-修复】新增步骤 1.5 安全约束检查**：将 Will Not 约束转化为可执行的拒绝规则，包含 5 类安全约束（绕过验证、直接覆盖、跳过历史、批量破坏、跨项目操作），违反时返回 `security_constraint_violation` 错误
- **【P0-修复】新增步骤 2.5 幂等性决策框架**：定义内容重复检测逻辑和决策矩阵（append/replace/update_table 三种模式 × 完全重复/语义重复/全新内容），禁止因"内容已存在"而中止执行询问用户，重复内容静默跳过但仍完成完整流程
- **【P1-修复】强化 force 模式更新日志同步**：在步骤 3.5 中显式强调 force 模式下即使目标为豁免章节也必须写入更新日志，禁止跳过
- **【P1-修复】强化验证步骤不可跳过约束**：在步骤 4 中增加防跳过声明，即使用户以紧急/高效为由也不得跳过验证
